...
- 사용자의 **개인 재산/자산 규모, risk level(위험 성향), 투자 성향**을 묻는 질문은
  ...
  - 예: "저대로 내 자산을 바꿔줘", "헷지 비율 적용해줘" (이전 대화에서 계산된 헷지 비율을 적용하여 자산 업데이트)

- **현재 상황에서 적절한 헷지 비율을 계산**하는 요청은
  → `strategy_execute` 에이전트에 라우팅합니다.
  - 예: "현재 내 상황에서, 적절한 헤지비율은 얼마일까?", "내 자산에 맞는 헷지 비율을 계산해줘", "최적 헷지 비율이 뭐야?"
